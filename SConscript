...
# ESP32C61
if GetDepend(['AT_DEVICE_USING_ESP32C61']):
    path += [cwd + '/class/esp32c61']
    src += Glob('class/esp32c61/at_device_esp32c61.c')
    if GetDepend(['AT_USING_SOCKET']):
        src += Glob('class/esp32c61/at_socket_esp32c61.c')
    if GetDepend(['AT_DEVICE_ESP32C61_SAMPLE']):
        src += Glob('samples/at_sample_esp32c61.c')
...
# NL668
if GetDepend(['AT_DEVICE_USING_NL668']):
    path += [cwd + '/class/nl668']
    src += Glob('class/nl668/at_device_nl668.c')
    if GetDepend(['AT_USING_SOCKET']):
        src +=Glob('class/nl668/at_socket_nl668.c')
    if GetDepend(['AT_DEVICE_NL668_SAMPLE']):
        src +=Glob('samples/at_sample_nl668.c')
# ML305
