...
#define LOG_TAG                     "at.skt.nl668"
#include <at_log.h>

#define AT_USING_SOCKET
#define AT_DEVICE_USING_NL668

#if defined(AT_DEVICE_USING_NL668) && defined(AT_USING_SOCKET)

#define NL668_MODULE_SEND_MAX_SIZE   2048
static int nl668_socket_fd[AT_DEVICE_NL668_SOCKETS_NUM] = {-1};

/* set real event by current socket and current state */
#define SET_EVENT(socket, event)       (((socket + 1) << 16) | (event))

/* AT socket event type */
#define NL668_EVENT_CONN_OK          (1L << 0)
#define NL668_EVENT_SEND_OK          (1L << 1)
#define NL668_EVENT_RECV_OK          (1L << 2)
#define NL668_EVNET_CLOSE_OK         (1L << 3)
#define NL668_EVENT_CONN_FAIL        (1L << 4)
#define NL668_EVENT_SEND_FAIL        (1L << 5)
...
static int nl668_get_socket_idx(int sock)
{
    int i;
    ...
    for (i=0; i<AT_DEVICE_NL668_SOCKETS_NUM; i++)
    {
        if (nl668_socket_fd[i] == sock)
            return(i);
    }
...
static int nl668_socket_event_send(struct at_device *device, uint32_t event)
{
    return (int) rt_event_send(device->socket_event, event);
}

static int nl668_socket_event_recv(struct at_device *device, uint32_t event, uint32_t timeout, rt_uint8_t option)
{
    int result = RT_EOK;
    rt_uint32_t recved;
 ...
 *         -5: no memory
 */
static int nl668_socket_close(struct at_socket *socket)
{
    uint32_t event = 0;
    int result = RT_EOK;
    ...
    if (nl668_socket_fd[device_socket] == -1)
    {
        return RT_EOK;
    }
    device_socket_id=nl668_socket_fd[device_socket];
    /* clear socket close event */
    event = SET_EVENT(device_socket_id, NL668_EVNET_CLOSE_OK);
    nl668_socket_event_recv(device, event, 0, RT_EVENT_FLAG_OR);
    ...
    if (nl668_socket_event_recv(device, event, rt_tick_from_millisecond(300*3), RT_EVENT_FLAG_AND) < 0)
    {
...
static int nl668_socket_connect(struct at_socket *socket, char *ip, int32_t port,enum at_socket_type type, rt_bool_t is_client)
{
    ...
    if (nl668_socket_fd[device_socket] != -1)
    {
        nl668_socket_fd[device_socket] = -1;
    }
    ...
    nl668_socket_fd[device_socket] = sock;
 ...
 *          -5: no memory
 */
static int nl668_socket_send(struct at_socket *socket, const char *buff, size_t bfsz, enum at_socket_type type)
{
    uint32_t event = 0;
    ...
    int sock = -1;
       ...
       sock=nl668_socket_fd[device_socket];
        if(sock<=0)
        {
            LOG_E("error socket ");
            return -RT_ERROR;
    ...
    event = SET_EVENT(sock, NL668_EVENT_SEND_OK | NL668_EVENT_SEND_FAIL);
    nl668_socket_event_recv(device, event, 0, RT_EVENT_FLAG_OR);
    ...
    {
        if (bfsz - sent_size < NL668_MODULE_SEND_MAX_SIZE)
        {
            cur_pkt_size = bfsz - sent_size;
        }
        else
        {
            cur_pkt_size = NL668_MODULE_SEND_MAX_SIZE;
        }
        ...
        if (nl668_socket_event_recv(device, SET_EVENT(sock, 0), 15 * RT_TICK_PER_SECOND, RT_EVENT_FLAG_OR) < 0)
        {
        ...
        event_result = nl668_socket_event_recv(device,
                NL668_EVENT_SEND_OK | NL668_EVENT_SEND_FAIL, 5 * RT_TICK_PER_SECOND, RT_EVENT_FLAG_OR);
        if (event_result < 0)
        {
        ...
        /* check result */
        if (event_result & NL668_EVENT_SEND_FAIL)
        {
 ...
 *         -5: no memory
 */
static int nl668_domain_resolve(const char *name, char ip[16])
{
    int result;
    at_response_t resp = RT_NULL;
 ...
 */
static void nl668_socket_set_event_cb(at_socket_evt_t event, at_evt_cb_t cb)
{
...
static void urc_send_func(struct at_client *client, const char *data, rt_size_t size)
{
    ...
    if (rt_strstr(data, "+MIPPUSH: ")){
        if(result==0)
        {
            nl668_socket_event_send(device, SET_EVENT(device_socket, NL668_EVENT_SEND_OK));
        }
        else
        {
            nl668_socket_event_send(device, SET_EVENT(device_socket, NL668_EVENT_SEND_FAIL));
        }
    }
...
static void urc_close_func(struct at_client *client, const char *data, rt_size_t size)
{
    ...
    if(result==0)
    {
        nl668_socket_event_send(device, SET_EVENT(device_socket, NL668_EVNET_CLOSE_OK));
    }
    else
    {
...
static void urc_recv_func(struct at_client *client, const char *data, rt_size_t size)
{
    ...
    device_socket = nl668_get_socket_idx(sock);
...
/*  nl668 device URC table for the socket data */
...
static const struct at_socket_ops nl668_socket_ops =
{
    nl668_socket_connect,
    nl668_socket_close,
    nl668_socket_send,
    nl668_domain_resolve,
    nl668_socket_set_event_cb,
...
int nl668_socket_class_register(struct at_device_class *class)
{
    ...
    class->socket_num = AT_DEVICE_NL668_SOCKETS_NUM;
    class->socket_ops = &nl668_socket_ops;

    return RT_EOK;
}

#endif /* AT_DEVICE_USING_NL668 && AT_USING_SOCKET */
