...
#include <at_device_esp32c61.h>

#define LOG_TAG                        "at.dev.esp32c61"

#include <at_log.h>

#ifdef AT_DEVICE_USING_ESP32C61

#define ESP32C61_WAIT_CONNECT_TIME      5000
#define ESP32C61_THREAD_STACK_SIZE      2048
#define ESP32C61_THREAD_PRIORITY        (RT_THREAD_PRIORITY_MAX / 2)
unsigned int ESP32C61_GMR_AT_VERSION;

/* =============================  esp32c61 network interface operations ============================= */

static int esp32c61_netdev_set_dns_server(struct netdev *netdev, uint8_t dns_num, ip_addr_t *dns_server);

static void esp32c61_get_netdev_info(struct rt_work *work, void *work_data)
{
...
#define AT_DEF_DNS_SERVER    "114.114.114.114"
    ...
    else
    {
        ...
        esp32c61_netdev_set_dns_server(netdev, 0, &ip_addr);
    }
...
static int esp32c61_net_init(struct at_device *device);

static int esp32c61_netdev_set_up(struct netdev *netdev)
{
...
static int esp32c61_netdev_set_down(struct netdev *netdev)
{
    struct at_device *device = RT_NULL;
...
static int esp32c61_netdev_set_addr_info(struct netdev *netdev, ip_addr_t *ip_addr, ip_addr_t *netmask, ip_addr_t *gw)
{
...
static int esp32c61_netdev_set_dhcp(struct netdev *netdev, rt_bool_t is_enabled)
{
#define ESP32C61_STATION       1 << 0
#define ESP32C61_SOFTAP        1 << 1
#define RESP_SIZE           128
    ...
    if (at_obj_exec_cmd(device->client, resp, "AT+CWDHCP=%d,%d", is_enabled, ESP32C61_STATION) < 0)
    {
...
static int esp32c61_netdev_ping(struct netdev *netdev, const char *host,
...
{
#define ESP32C61_PING_IP_SIZE         16

    rt_err_t result = RT_EOK;
    at_response_t resp = RT_NULL;
    struct at_device *device = RT_NULL;
    char ip_addr[ESP32C61_PING_IP_SIZE] = {0};
    int req_time;
    ...
    if (at_resp_parse_line_args_by_kw(resp, "+CIPDOMAIN:", (esp32c61_get_at_version() <= ESP32C61_DEFAULT_AT_VERSION_NUM) ? \
    ...
    if (at_resp_parse_line_args_by_kw(resp, "+", (esp32c61_get_at_version() <= ESP32C61_DEFAULT_AT_VERSION_NUM) ?
                                      "+%d" : "+PING:%d", &req_time) < 0)
    {
        result = -RT_ERROR;
        goto __exit;
...
void esp32c61_netdev_netstat(struct netdev *netdev)
{
#define ESP32C61_NETSTAT_RESP_SIZE         320
#define ESP32C61_NETSTAT_TYPE_SIZE         4
#define ESP32C61_NETSTAT_IPADDR_SIZE       17
#define ESP32C61_NETSTAT_EXPRESSION        "+CIPSTATUS:%*d,\"%[^\"]\",\"%[^\"]\",%d,%d,%*d"
    ...
    type = (char *) rt_calloc(1, ESP32C61_NETSTAT_TYPE_SIZE);
    ipaddr = (char *) rt_calloc(1, ESP32C61_NETSTAT_IPADDR_SIZE);
    ...
    resp = at_create_resp(ESP32C61_NETSTAT_RESP_SIZE, 0, 5 * RT_TICK_PER_SECOND);
    if (resp == RT_NULL)
    {
    ...
    for (i = 1; i <= resp->line_counts; i++)
    {
        if (strstr(at_resp_get_line(resp, i), "+CIPSTATUS"))
        {
            ...
            if (at_resp_parse_line_args(resp, i, ESP32C61_NETSTAT_EXPRESSION, type, ipaddr, &remote_port, &local_port) < 0)
            {
                goto __exit;
            }
            else
...
static const struct netdev_ops esp32c61_netdev_ops =
{
    esp32c61_netdev_set_up,
    esp32c61_netdev_set_down,

    esp32c61_netdev_set_addr_info,
    esp32c61_netdev_set_dns_server,
    esp32c61_netdev_set_dhcp,

#ifdef NETDEV_USING_PING
    esp32c61_netdev_ping,
#endif
#ifdef NETDEV_USING_NETSTAT
    esp32c61_netdev_netstat,
#endif
};

static struct netdev *esp32c61_netdev_add(const char *netdev_name)
{
...
#define HWADDR_LEN          6
    ...
    netdev->ops = &esp32c61_netdev_ops;
...
static void esp32c61_netdev_start_delay_work(struct at_device *device)
{
    ...
    rt_work_init(net_work, esp32c61_get_netdev_info, (void *)device);
    rt_work_submit(net_work, RT_TICK_PER_SECOND);
}

static void esp32c61_init_thread_entry(void *parameter)
{
#define INIT_RETRY    5

    struct at_device *device = (struct at_device *) parameter;
    struct at_device_esp32c61 *esp32c61 = (struct at_device_esp32c61 *)rt_container_of(device, struct at_device_esp32c61, device);
    struct at_client *client = device->client;
    at_response_t resp = RT_NULL;
    rt_err_t result = RT_EOK;
    ...
    /* wait esp32c61 device startup finish */
    if (at_client_obj_wait_connect(client, ESP32C61_WAIT_CONNECT_TIME))
    {
        return;
    }
    ...
    while (retry_num--)
    {
        ...
        ESP32C61_GMR_AT_VERSION = esp32c61_at_version_to_hex(at_resp_get_line(resp, 1));
        /* show module version */
    ...
    if (at_obj_exec_cmd(client, at_resp_set_info(resp, 512, 0, 20 * RT_TICK_PER_SECOND),
                        "AT+CWJAP=\"%s\",\"%s\"", esp32c61->wifi_ssid, esp32c61->wifi_password) != RT_EOK)
    {
        LOG_W("%s device wifi connect failed, check ssid(%s) and password(%s).",
              device->name, esp32c61->wifi_ssid, esp32c61->wifi_password);
    }
    else
    {
    ...
    else
    {
        ...
        esp32c61_netdev_start_delay_work(device);
...
static int esp32c61_net_init(struct at_device *device)
{
#ifdef AT_DEVICE_ESP32C61_INIT_ASYN
    rt_thread_t tid;

    tid = rt_thread_create("esp61_net", esp32c61_init_thread_entry, (void *) device,
            ESP32C61_THREAD_STACK_SIZE, ESP32C61_THREAD_PRIORITY, 20);
    if (tid)
    {
...
#else
    esp32c61_init_thread_entry(device);
#endif /* AT_DEVICE_ESP32C61_INIT_ASYN */
...
static int esp32c61_init(struct at_device *device)
{
    struct at_device_esp32c61 *esp32c61 = rt_container_of(device, struct at_device_esp32c61, device);

    /* initialize AT client */
#if RT_VER_NUM >= 0x50100
    at_client_init(esp32c61->client_name, esp32c61->recv_line_num, esp32c61->recv_line_num);
#else
    at_client_init(esp32c61->client_name, esp32c61->recv_line_num);
#endif

    device->client = at_client_get(esp32c61->client_name);
    if (device->client == RT_NULL)
    {
        LOG_E("get AT client(%s) failed.", esp32c61->client_name);
        return -RT_ERROR;
    }
    ...
    esp32c61_socket_init(device);
#endif

    /* add esp32c61 device to the netdev list */
    device->netdev = esp32c61_netdev_add(esp32c61->device_name);
    if (device->netdev == RT_NULL)
    {
        LOG_E("add netdev(%s) failed.", esp32c61->device_name);
        return -RT_ERROR;
    }

    /* initialize esp32c61 device network */
    return esp32c61_netdev_set_up(device->netdev);
}

static int esp32c61_deinit(struct at_device *device)
{
    return esp32c61_netdev_set_down(device->netdev);
}
/**
 * Set the host name of the ESP32C61 device.
 *
 * @param device    Pointer to the AT device structure.
 * @param host_name Host name string to set for the ESP32C61 device.
 *
 * @return RT_EOK on success, -RT_ERROR on failure.
 */
static int esp32c61_set_host_name(struct at_device *device, const char *host_name)
{
    int result = RT_EOK;
    struct at_client *client = device->client;
...
/* reset esp32c61 device and initialize device network again */
static int esp32c61_reset(struct at_device *device)
{
    int result = RT_EOK;
    struct at_client *client = device->client;

    /* send "AT+RST" commonds to esp32c61 device */
    result = at_obj_exec_cmd(client, RT_NULL, "AT+RST");
    rt_thread_mdelay(1000);

    /* waiting 10 seconds for esp32c61 device reset */
    device->is_init = RT_FALSE;
    if (at_client_obj_wait_connect(client, ESP32C61_WAIT_CONNECT_TIME))
    {
        return -RT_ETIMEOUT;
...
/* change esp32c61 wifi ssid and password information */
static int esp32c61_wifi_info_set(struct at_device *device, struct at_device_ssid_pwd *info)
{
    int result = RT_EOK;
...
static int esp32c61_control(struct at_device *device, int cmd, void *arg)
{
    int result = -RT_ERROR;
    ...
    case AT_DEVICE_CTRL_RESET:
        result = esp32c61_reset(device);
        break;
    case AT_DEVICE_CTRL_SET_WIFI_INFO:
        result = esp32c61_wifi_info_set(device, (struct at_device_ssid_pwd *) arg);
        break;
    case AT_DEVICE_CTRL_SET_HOST_NAME:
        result = esp32c61_set_host_name(device, (const char *)arg);
        break;
    default:
...
static const struct at_device_ops esp32c61_device_ops =
{
    esp32c61_init,
    esp32c61_deinit,
    esp32c61_control,
...
static int esp32c61_device_class_register(void)
{
    ...
    /* fill ESP32C61 device class object */
#ifdef AT_USING_SOCKET
    esp32c61_socket_class_register(class);
#endif
    class->device_ops = &esp32c61_device_ops;

    return at_device_class_register(class, AT_DEVICE_CLASS_ESP32C61);
}
INIT_DEVICE_EXPORT(esp32c61_device_class_register);
...
/**
 * This function is used to obtain the ESP32C61 AT version number.
 *
 * @return hex_number: Hexadecimal AT version number, example: 1.4.1.1 -> 0x1040101
 */
unsigned int esp32c61_get_at_version(void)
{
    return ESP32C61_GMR_AT_VERSION;
}
...
#endif /* AT_DEVICE_USING_ESP32C61 */