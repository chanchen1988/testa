...
#define LOG_TAG                       "at.skt.esp32c61"
#include <at_log.h>

#if defined(AT_DEVICE_USING_ESP32C61) && defined(AT_USING_SOCKET)

#define ESP32C61_MODULE_SEND_MAX_SIZE   2048
/* set real event by current socket and current state */
#define SET_EVENT(socket, event)       (((socket + 1) << 16) | (event))

/* AT socket event type */
#define ESP32C61_EVENT_CONN_OK          (1L << 0)
#define ESP32C61_EVENT_SEND_OK          (1L << 1)
#define ESP32C61_EVENT_RECV_OK          (1L << 2)
#define ESP32C61_EVNET_CLOSE_OK         (1L << 3)
#define ESP32C61_EVENT_CONN_FAIL        (1L << 4)
#define ESP32C61_EVENT_SEND_FAIL        (1L << 5)
...
static int esp32c61_socket_event_send(struct at_device *device, uint32_t event)
{
    return (int) rt_event_send(device->socket_event, event);
}

static int esp32c61_socket_event_recv(struct at_device *device, uint32_t event, uint32_t timeout, rt_uint8_t option)
{
    int result = 0;
    rt_uint32_t recved;
 ...
 *         -5: no memory
 */
static int esp32c61_socket_close(struct at_socket *socket)
{
    int result = RT_EOK;
 ...
 *          -5: no memory
 */
static int esp32c61_socket_connect(struct at_socket *socket, char *ip, int32_t port, enum at_socket_type type, rt_bool_t is_client)
{
    int result = RT_EOK;
...
__retry:
    ...
    if (result != RT_EOK && retryed == RT_FALSE)
    {
        ...
        if (esp32c61_socket_close(socket) < 0)
        {
            goto __exit;
        }
        retryed = RT_TRUE;
 ...
 */
int esp32c61_socket_listen(struct at_socket *socket, int backlog)
{
    int result               = RT_EOK;
    at_response_t resp       = RT_NULL;
    struct at_device *device = RT_NULL;
 ...
 *          -5: no memory
 */
static int esp32c61_socket_send(struct at_socket *socket, const char *buff, size_t bfsz, enum at_socket_type type)
{
    int result = RT_EOK;
    int event_result = 0;
    ...
    esp32c61->urc_socket = device_socket;
    ...
    {
        if (bfsz - sent_size < ESP32C61_MODULE_SEND_MAX_SIZE)
        {
            cur_pkt_size = bfsz - sent_size;
        }
        else
        {
            cur_pkt_size = ESP32C61_MODULE_SEND_MAX_SIZE;
        }
        ...
        if (esp32c61_socket_event_recv(device, SET_EVENT(device_socket, 0),
        ...
        event_result = esp32c61_socket_event_recv(device, ESP32C61_EVENT_SEND_OK | ESP32C61_EVENT_SEND_FAIL,
        ...
        /* check result */
        if (event_result & ESP32C61_EVENT_SEND_FAIL)
        {
 ...
 *         -5: no memory
 */
static int esp32c61_domain_resolve(const char *name, char ip[16])
{
#define RESOLVE_RETRY        5
    ...
    for (i = 0; i < RESOLVE_RETRY; i++)
    {
        ...
        if (at_resp_parse_line_args_by_kw(resp, "+CIPDOMAIN:", (esp32c61_get_at_version() <= ESP32C61_DEFAULT_AT_VERSION_NUM) ? "+CIPDOMAIN:%s" : "+CIPDOMAIN:\"%[^\"]\"", recv_ip) < 0)
        {
 ...
 */
static void esp32c61_socket_set_event_cb(at_socket_evt_t event, at_evt_cb_t cb)
{
...
static const struct at_socket_ops esp32c61_socket_ops =
{
    esp32c61_socket_connect,
    esp32c61_socket_close,
    esp32c61_socket_send,
    esp32c61_domain_resolve,
    esp32c61_socket_set_event_cb,
#if defined(AT_SW_VERSION_NUM) && AT_SW_VERSION_NUM > 0x10300
    RT_NULL,
#endif
#ifdef AT_USING_SOCKET_SERVER
    esp32c61_socket_listen,
#endif
};
...
static void urc_send_func(struct at_client *client, const char *data, rt_size_t size)
{
    int device_socket = 0;
    struct at_device *device = RT_NULL;
    struct at_device_esp32c61 *esp32c61 = RT_NULL;
    char *client_name = client->device->parent.name;
    ...
    esp32c61         = rt_container_of(device, struct at_device_esp32c61, device);
    device_socket = esp32c61->urc_socket;

    if (rt_strstr(data, "SEND OK"))
    {
        esp32c61_socket_event_send(device, SET_EVENT(device_socket, ESP32C61_EVENT_SEND_OK));
    }
    else if (rt_strstr(data, "SEND FAIL"))
    {
        esp32c61_socket_event_send(device, SET_EVENT(device_socket, ESP32C61_EVENT_SEND_FAIL));
    }
}
...
int esp32c61_socket_class_register(struct at_device_class *class)
{
    ...
    class->socket_num = AT_DEVICE_ESP32C61_SOCKETS_NUM;
    class->socket_ops = &esp32c61_socket_ops;

    return RT_EOK;
}

#endif /* AT_DEVICE_USING_ESP32C61 && AT_USING_SOCKET */